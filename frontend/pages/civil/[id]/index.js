...
export default function CivilQuoteDetail() {
  ...
  if (!quote) return <Layout><div className="text-center py-20">견적서를 찾을 수 없습니다</div></Layout>

  return (
    <Layout title={`🚜 ${quote.field_name || '토목 견적서'}`}>
      <div className="mb-3">
        <button onClick={() => router.push('/civil')} className="text-sm text-gray-500 hover:text-gray-700">
          ← 목록으로
        </button>
      </div>
      <QuoteView quote={quote} domainName="토목" onEdit={() => router.push(`/civil/${id}/edit`)} />
    </Layout>
  )
}
