    ...
    <Layout title="🚜 토목 견적서">
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <Link href="/civil/new"
          className="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2
            rounded-lg transition-colors text-center whitespace-nowrap">
          + 새 견적서 작성
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          ...
          <p className="text-gray-500">작성된 토목 견적서가 없습니다</p>
          <Link href="/civil/new"
            className="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2
              rounded-lg transition-colors inline-block mt-4">
            첫 견적서 작성하기
          </Link>
        </div>
      ) : (
                  ...
                  <tr key={q.id} className="hover:bg-green-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium
                        ${q.status === 'confirmed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        {q.status === 'confirmed' ? '확정' : '작성중'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2 justify-end">
                        <Link href={`/civil/${q.id}`} className="text-green-600 hover:text-green-800 text-xs">보기</Link>
                        <Link href={`/civil/${q.id}/edit`} className="text-gray-500 hover:text-gray-700 text-xs">수정</Link>
