...
export default function EditArchitectureQuote() {
  ...
  const initialRows = quote.items?.map(item => ({
    ...item,
    _id: item.id,
    is_weight_based: !!item.unit_weight,
  })) || []

  return (
    <Layout title="🏛 건축 견적서 수정">
      <div className="mb-3">
        <button onClick={() => router.push(`/architecture/${id}`)}
      ...
      <QuoteEditor
        ...
        initial={{
          header: {
            ...
            note: quote.note,
            labor_pct: quote.labor_pct,
            expense_pct: quote.expense_pct,
            management_pct: quote.management_pct,
